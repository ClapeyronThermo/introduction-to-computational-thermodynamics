...
begin
	...
	const R = R̄
	using Clapeyron, ForwardDiff, LinearAlgebra, NLsolve, Optimization, OptimizationOptimJL, Optim
	using Statistics
...
Let's suppose we have a preexisting mixture with a composition $\mathbf z$. The change in Gibbs free energy upon the formation of a small amount, $\delta e$, of a new phase with composition $\mathbf x$, can be written as

$$\delta G = \delta e \sum x_i(\mu_i(\mathbf x) - \mu_i(\mathbf z)),$$

where $\mu_i$ represents the chemical potential of species $i$.

For the preexisting mixture to be stable, it must lie at the minimum of the Gibbs free energy. This means that the formation of a new phase will increase $G$, so we must therefore have that
...
If $TPD \geq 0$ for all possible compositions $\mathbf x$, then our mixture must be stable, i.e., no new phase will appear. Another way of looking at this is that if we evaluate every minimum of $TPD$, and they are all positive, then the mixture is stable.
...
function chemical_stability_analysis(model, p, T, z)
	...
	optf = OptimizationFunction((u,_)->f(u), Optimization.AutoForwardDiff())
	sols = zeros(2,length(z))
	vals = zeros(2)
	for i in eachindex(x0_vec)
		prob = optimize(f, x0_vec[i], Newton(); autodiff = :forward)
		sols[i,:] = Optim.minimizer(prob)
		vals[i] = f(sols[i,:])
		# Evaluate solutions
	end

	# Extract minimum
	tm_min, idx = findmin(vals)
	# W = exp.(sol_vec[idx].minimizer)
	tm_xmin = normalize(exp.(sols[idx,:]), 1)
...
let
    ...
    plot!(x_range, gᴹ.(z_range), label=L"$Δg$ surface", linewidth=2)
    annotate!(0.77, 0.23, text(L"T=%$(T)~\mathrm{K}"*"\n"*L"p=%$(p/1e6)~\mathrm{MPa}", :black, :left, 14))

    plot!([flash_xmin[1], flash_ymin[1]], [gᴹ(flash_xmin), gᴹ(flash_ymin)], label="equilibrium tangent plane", linewidth=3, linestyle=:dash)

	tm_xmin, tpd_min = chemical_stability_analysis(model, p, T, z)
	scatter!([tm_xmin[1]], [gᴹ(tm_xmin)], markersize=4, markercolour=:black, label=L"initial guess ($tm$ minimum)")
	fig9
end
...
let
    ...
    plot!(x_range, gᴹ.(z_range), label=L"$Δg$ surface", linewidth=2)
    annotate!(0.71, 0.08, text(L"T=%$(T)~\mathrm{K}"*"\n"*L"p=%$(p/1e6)~\mathrm{MPa}", :black, :left, 14))
...
manifest_format = "2.0"
project_hash = "57e0bed1200557d98df580b3ca5f99ec80e351f6"
...
git-tree-sha1 = "bbf9931006a0143015ac57ec1a896efc80ffb26a"
...
git-tree-sha1 = "2e97db19d321e28ed26c93a44bd01ad225b7391a"

...
md"""
Another technique often adopted is using logs to scale variables scanning multiple orders of magnitude. This frequently arises in relation to volumes, with liquid and vapour volumes varying by up to 1000×.
"""
...
however, as we iterate using two variables (the specific volumes of each phase), we select only equations (4) and (5) to complete our root-finding problem. (To be strict, once the solution has been found, equation (6) should also be checked for consistency. In practice, however, this normally is not done.)
...
$$V_0^\mathrm{liq} = \frac{1}{0.3}\cdot\frac{\pi}{6}\cdot N_\mathrm{A} \cdot \sigma^3$$
...
let
	...
	gr()
	plot(title=L"$pT$ plot for water", xlabel=L"T / \mathrm{K}", ylabel=L"p / \mathrm{MPa}", framestyle=:box, tick_direction=:out, grid=:off, legend=:topleft)
...
let
	Tc, pc, Vc = crit_pure(cubic_model)
	gr()
	plot(xaxis=:log, title=L"$VT$ plot for water", xlabel=L"V / \mathrm{m^3}", ylabel=L"T / \mathrm{K}", framestyle=:box, tick_direction=:out, grid=:off)
...
manifest_format = "2.0"
project_hash = "854615e89f8ade87016b1a11410097e338d6802e"

...
Usually, we specify temperature and pressure ($T,p$) which, when solving for the volume, corresponds to solving the equation
...
where the values of $\delta_1, \delta_2$ are specific to each equation of state (van der Waals, Peng–Robinson, Redlich–Kwong), and the form of the **$\alpha$ function**, $\alpha(T)$, can be selected to match specific types of components or problems. In this workbook we will consider the van der Waals EoS, which is the case when
...
let
	...
	gr()
	plt = plot(
		title=L"$pv$ diagram for CO₂ using the van der Waals EoS",
		ylabel=L"p / \mathrm{MPa}", xlabel=L"v / (\mathrm{m^3/mol})",
...
md"""
Now we have that working, we can see we have three real roots. How do we know which one to choose? We know that the smallest root should correspond to a _liquid_-like specific volume, while the largest root corresponds to a _vapour_-like specific volume, and that the middle root has no physical meaning. From our physical knowledge of carbon dioxide, we can tell that it should be a gas and so we should take the vapour-like root, but this isn't something we can apply rigorously or put into our code.
...
With SAFT (e.g. ogSAFT or SAFT-VR Mie) or related equations of state, we have an expression for the residual Helmholtz free energy. This is written as a sum of different contributions
...
One way to obtain an expression we could solve for the volume at a specified pressure, and temperature would be to take the partial derivative of $a$ to express this as a non-linear equation
...
$$1 = \frac{\frac{\pi}{6}\cdot N_\mathrm{A}\cdot \sigma^3}{V},$$

giving us the expression for our volume guess as

$$V_0^\mathrm{liq} = \frac{\pi}{6}\cdot N_\mathrm{A} \cdot \sigma^3.$$
"""
...
let
	...
	gr()
	plt = plot(
		title=L"$pv$ diagram for CO₂ using the PC-SAFT EoS",
		ylabel=L"p / \mathrm{MPa}", xlabel=L"v / (\mathrm{m^3/mol})",
		framestyle=:box, tick_direction=:out, grid=:off,
		xscale=:log,
		ylim = (minimum(psat), 1.1pcrit)
...
Generates initial guesses for the volume solver. The liquid phase initial guess is based on the packing fraction limit, and the vapour phase guess is based on the ideal gas equation.
"""
...
let
	...
	plt = plot(
		title="",
		ylabel=L"f(V) / 10¹⁰", xlabel=L"v / (10⁻⁵ (\mathrm{m^3/mol}))",
...
The only issue encountered with this is that we need to rethink our approach to the generation of initial guesses. The approach used typically differs according to the "class" of the equation of state in use.
"""
...
manifest_format = "2.0"
project_hash = "a2b22290e12ddda9a651306bc3544769f1350703"

...
let
	...
	plot(title="The Rachford–Rice equation", xlabel=L"β", ylabel=L"f(β)", framestyle=:box, xlim=(βmin, βmax), ylim=(-4, 4), tick_direction=:out, grid=:off, legendfont=font(10), xtick=-0.6:0.2:1.6)

	plot!(β_vec, rr_plot.(β_vec), label="", linewidth=2)
	vline!([0.0, 1.0], label=L"physical domain of $β$", linewidth=2)
...
where $K_\max$ and $K_\min$ represent the maximum and minimum values of the K-factors.
...
$$β_\mathrm{new} ≥ β_\max$$

or

$$β_\mathrm{new} \leq β_\min$$
...
$$\beta_\min < \beta_\mathrm{new} < \beta_\max.$$
...
@htl("""
<table>
	<caption><i>z</i> specification</caption>
  <tr>
	<th>Component</th>
	<th>Mole fraction</th>
  </tr>
...
# ╔═╡ e725bda0-222e-47ee-8e1b-f9ef320aec7b
@htl("""
<table>
	<caption><i>pT</i> specification</caption>
  <tr>
	<th>Pressure</th>
	<td>60 bar</td>
  </tr>
...
This is the same initialisation procedure we used for our **stability analysis** algorithm. Another possibility would be to use the result from stability analysis as an initial guess. This can be valuable as good initial guesses are particularly important when avoiding converging to the **trivial solution**.
"""
...
# ╔═╡ 1c14bee3-a026-4fbd-88cb-a3e909a66377
@htl("""
z<sub>methane</sub> =
""")

# ╔═╡ 236860b9-ba77-4fa2-b28c-27c97f202aed
@bind z_methane PlutoUI.Slider(0:0.05:1, show_value=true, default=0.8)
...
x_Clapeyron, n_Clapeyron, G = tp_flash(model, p, T, z, RRTPFlash())
...
Accelerated successive substitution has **superlinear** convergence. This is when our convergence is faster than linear, but not yet quadratic. The acceleration procedure uses the last $m$ iterations, in this case $m=5$, to extrapolate each step and converge faster.
...
manifest_format = "2.0"
project_hash = "0723a237fda44ff2a456cc248535f155a502a3b2"

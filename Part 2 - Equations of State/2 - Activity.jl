...
$$R_\mathrm{A,evap.} = k_\mathrm{A,evap.}nx_\mathrm{A}.$$

And the rate of condensation is (the gas obeys Dalton's law)

$$R_\mathrm{A,cond.} = k_\mathrm{A,cond.}py_\mathrm{A}$$

giving

$$k_\mathrm{A,cond.}py_\mathrm{A} = k_\mathrm{A,evap.}nx_\mathrm{A}.$$

If we rearrange this expression and the expression for the pure, we find

$$\frac{k_\mathrm{A,cond.}}{k_\mathrm{A,evap.}n} = \frac{x_\mathrm{A}}{y_\mathrm{A}p} = \frac{1}{p_\mathrm{A,sat.}}.$$
...
where $x_{ji}$ is the local composition of species $j$ around species $i$ and $v_i$ is the molar volume of a pure species $i$. Unsurprisingly, it is not possible to obtain the $x_{ji}$ experimentally. As such, Wilson made the assumption

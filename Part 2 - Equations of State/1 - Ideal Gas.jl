...
As a common starting ground, let us examine an equation of state most of us will have encountered during our time in highschool: the Ideal Gas equation, initially formulated by Émille Clapeyron. Typically, it is written as
...
md"""
One might think that, because of its simple nature, the ideal gas is not suitable for most practical applications. However, not only do simple spherical species (such as noble gases and methane) agree well with it, even typically complex species, such as water, eventually begin to behave ideally. This is, of course, limited to the gas phase at low pressures. For certain applications, this may be sufficient.
...
begin
	plot(T,Cp1/8.314,xlim=(100,3000),ylim=(3,18),
		...
		ylabel=L"c_p / R", xlabel=L"T / \mathrm{K}")
...
md"""
In the above, we use the theoretically-rigorous approach by Walker and Haslam (`WalkerIdeal`), which always ensures the high and low temperature limits are met, and the empirical Reid polynomial (`ReidIdeal`). As we can see, without considering experimental data, the `ReidIdeal` model doesn't satisfy the high (and low) temperature limits, meaning we should only use this model around intermediate temperature (300 K to 1000 K) which is where most of our systems should be anyways.
"""

# ╔═╡ da8dfd20-5c83-4174-9f53-19f9a84cc8b4
md"""
In terms of ideal gas models, there are many to choose from, but generally fall into two categories: 1. Species-specific and 2. Group contribution. As previously mentioned, Reid polynomials, a species-specific equation, obtain the molar ideal heat capacity as
...
Whilst this method is quite simple, one of the failings of group contribution is, given we are now trying to fit a larger dataset with fewer parameters, the accuracy will inevitably be reduced, in comparison to species-specific correlations. However, the accuracy of the method can generally be improved based on how the groups are selected; if a group covers a larger set of atoms, we can expect that it will provide a better estimate of the ideal heat capacity. For example, when modelling ethanol, using CH₃ and CH₂OH groups instead of CH₃, CH₂, and OH groups would most likely result in more accurate estimates of the heat capacity. This argument holds true for any group-contribution methodology.
...
begin
	...
	plot(T1,Y1,xlim=(250,500),
		...
		ylabel=L"C_p / (\mathrm{J/(K mol)})", xlabel=L"T / \mathrm{K}",)
